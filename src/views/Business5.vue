      ...
      <el-dialog title="结价订单" :visible.sync="dialogFormVisible1" width="30%">
        <el-form :model="endOrder">
          ...
          <el-form-item label="结价克重:" :label-width="formLabelWidth">
            <el-input class="left" v-model="endOrder.getGram" autocomplete="off"></el-input>
          </el-form-item>
          <el-form-item label="结价价格:" :label-width="formLabelWidth">
            <el-input class="left" v-model="endOrder.endPrice" autocomplete="off"></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    updateEnd(){
      ...
      else if(this.endOrder.endPrice==null){
        this.$message({
          message: '结价价格不能为空',
          type: 'warning'
        });
    ...
    checkOrder(){
      ...
      else if(this.order.metal===''||this.order.metal==undefined){
        this.$message({
          message: '请选择产品',
          type: 'waring'
        });
      }
      else if(this.order.gram ==NaN || this.order.gram <0){
        this.$message({
          message: '产品克重不能为负数',
          type: 'waring'
        });
      }
      else if(this.order.deposit==null || this.order.deposit <0){
        this.$message({
          message: '订金不能为负数',
          type: 'waring'
        });
      }
      else if(this.order.rent==null || this.order.rent<0){
        this.$message({
          message: '每日租金不能小于0，且不能为空',
          type: 'waring'
        });
      }
      else return true;
